...
export function createQuadWireframe(geometry: THREE.BufferGeometry) {
  ...
  const key = (v: THREE.Vector3) =>
    [v.x, v.y, v.z]
      .map((value, axis) =>
        Math.round(((value - bounds.min.getComponent(axis)) / scale) * 1e6),
      )
      .join(",");
  const edges = new Map<
    string,
    { a: THREE.Vector3; b: THREE.Vector3; faces: number[]; length: number }
  >();
  ...
  for (let i = 0; i < count; i += 3) {
    ...
    const lengths = vertices.map((v, j) =>
      v.distanceToSquared(vertices[(j + 1) % 3]),
    );
  ...
  for (const edge of edges.values()) {
    ...
    if (
      ...
      !paired.has(a) &&
      !paired.has(b) &&
      faces[a].normal.dot(faces[b].normal) >
